            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover">
                    ...
                    <tr class="table-background-header">
                        ...
                        <th width="10%" class="text-left middle">{{viewLanguage('Đối tác')}}</th>
                        <th width="15%" class="text-left middle">{{viewLanguage('Chiến dịch')}}</th>

                        <th width="10%" class="text-center middle">{{viewLanguage('Gói')}}</th>
                        <th width="10%" class="text-center middle">{{viewLanguage('Code')}}</th>
                        <th width="10%" class="text-center middle">{{viewLanguage('Code active')}}</th>
                        <th width="10%" class="text-right middle">{{viewLanguage('Giá')}}</th>
                    ...
                    @foreach ($data as $key => $item)
                        <tr data-form-name="detailItem" data-input="{{json_encode([])}}" data-show="2" data-div-show="content-page-right" title="{{viewLanguage('Cập nhật: ')}}" data-method="get" data-url="" data-objectId="1">
                            ...
                            <td class="text-left middle">
                                ...
                                {{--@if(isset($item->STAFF_CODE))<b>Mã NS:</b> {{$item->STAFF_CODE}}<br/>@endif--}}
                                {{--@if(isset($item->IDCARD))<b>ID card:</b> {{$item->IDCARD}}<br/>@endif--}}
