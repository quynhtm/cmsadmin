            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover">
                    ...
                    <tr class="table-background-header">
                        ...
                        <th width="15%" class="text-center">{{viewLanguage('Tên đối tác')}}</th>
                        <th width="20%" class="text-left">{{viewLanguage('Url web')}}</th>
                        <th width="20%" class="text-left">{{viewLanguage('Thông tin thêm')}}</th>
                        <th width="30%" class="text-left">{{viewLanguage('Ghi chú')}}</th>
                    ...
                    @foreach ($data as $key => $item)
                        <tr>
                            ...
                            <td class="text-left middle">{{$item->partner_name}}</td>
                            <td class="text-left middle">{{$item->partner_web}}</td>

                            <td class="text-left middle">
                                @if(trim($item->partner_phone) != '')P: {{$item->partner_phone}} <br/> @endif
                                @if(trim($item->partner_email) != '')E: {{$item->partner_email}} <br/> @endif
                                @if(trim($item->partner_zalo) != '')Z: {{$item->partner_zalo}} <br/> @endif
                            </td>
