            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover">
                    ...
                    <tr class="table-background-header">
                        ...
                        <th width="20%" class="text-center">{{viewLanguage('Tên tin bài')}}</th>
                        <th width="12%" class="text-left">{{viewLanguage('Loại tin')}}</th>
                        <th width="12%" class="text-left">{{viewLanguage('Danh mục')}}</th>

                        <th width="30%" class="text-left">{{viewLanguage('Mô tả ngắn')}}</th>
                        <th width="5%" class="text-center">{{viewLanguage('Vị trí')}}</th>
                    ...
                    @foreach ($data as $key => $item)
                        <tr>
                            ...
                            <td class="text-left middle">
                                @if(isset($arrNewsType[$item->news_type])){{$arrNewsType[$item->news_type]}}@endif
                            </td>
                            <td class="text-left middle">
                                @if(isset($arrCategory[$item->news_category])){{$arrCategory[$item->news_category]}}@endif
                            </td>
                            <td class="text-left middle">{!! substring($item->news_desc_sort) !!}</td>
                            <td class="text-center middle">{{$item->news_order}}</td>
                            ...
                            <td class="text-center middle">
                                ...
                                @if($item->news_status == STATUS_INT_MOT)
