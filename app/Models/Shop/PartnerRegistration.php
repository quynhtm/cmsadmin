...
class PartnerRegistration extends BaseModel
{
    protected $table = TABLE_PARTNER_REGISTRATION;
    ...
    {
        try {
            $query = PartnerRegistration::where($this->primaryKey, '>', STATUS_INT_KHONG);
    ...
    public function editItem($data, $id = 0)
    {
        try {
            ...
            if (is_array($fieldInput) && count($fieldInput) > STATUS_INT_KHONG) {
                $item = ($id <= STATUS_INT_KHONG)? new PartnerRegistration(): self::getItemById($id);
                if (is_array($fieldInput) && count($fieldInput) > STATUS_INT_KHONG) {
    ...
    public function getItemById($id)
    {
        ...
        if (!$data) {
            $data = PartnerRegistration::find($id);
            if ($data) {
