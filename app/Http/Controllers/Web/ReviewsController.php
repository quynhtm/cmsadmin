...
class ReviewsController extends BaseAdminController
{
    ...
    private $routerIndex = 'reviewsProduct.index';
    private $templateRoot = DIR_PRO_WEB . '/' . 'Reviews.';

    public function __construct()
    {
        parent::__construct();
        $this->modelObj = new Reviews();
    ...
    private function _outDataView($request, $data)
    {
        ...
        return $this->dataOutCommon = [
            ...
            'urlGetData' => URL::route('reviews.ajaxGetData'),
            'urlPostData' => URL::route('reviews.ajaxPostData'),
        ];
    }

    public function indexReviewsProduct()
    {
        $this->pageTitle = CGlobal::$pageAdminTitle = 'Đánh giá sản phẩm';
        return $this->index('reviewsProduct.index', Reviews::typeReviewProduct);
    }

    public function indexReviewsNew()
    {
        $this->pageTitle = CGlobal::$pageAdminTitle = 'Bình luận tin tức';
        return $this->index('reviewsNew.index', Reviews::typeReviewNew);
    }

    public function index($urlIndex, $typeReview)
    {
        ...
        return view($this->templateRoot . 'viewIndex', array_merge($this->dataOutCommon,[
            'data' => $dataList,
            'total' => $total,
            'search' => $search,
            'stt' => ($page_no - 1) * $limit,
